...
import (
	...
	"github.com/Sam12121/package-scanner/jobs"
	"github.com/Sam12121/package-scanner/output"
...
func RunGrpcServer(pluginName string, config utils.Config) error {
	...
	config.ToaeKey = os.Getenv("TOAE_KEY")

	if dschttp.IsConsoleAgent(config.ConsoleURL) && strings.Trim(config.ToaeKey, "\"") == "" {
		...
		for {
			var err error
			if config.ToaeKey, err = dschttp.GetConsoleApiToken(internalURL, internalPort); err != nil {
				log.Error(err)
...
func processSbomGeneration(configInterface interface{}) interface{} {
	...
	// Send sbom to Toae Management Console for Vulnerability Scan
